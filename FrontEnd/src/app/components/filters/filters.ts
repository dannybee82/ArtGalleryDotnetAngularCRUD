...
import { AvailableFilter } from '../../services/filters/available-filter';
...
@Component({
  ...
  imports: [AllMatModules, FormsModule, ReactiveFormsModule],
...
export class Filters implements OnInit {
  ...
  readonly filterValues: OutputEmitterRef<FilterData | undefined> = output<
    ...
    for (let i = 0; i < controlNames.length; i++) {
      ...
      if (values) {
        for (let j = 0; j < values.length; j++) {
          const itemfound: FilterItem | undefined = structuredClone(targetArrs[i]).find(
            (item) => item.value === values[j],
          );
