...
@Component({
  ...
  templateUrl: './create-or-update-artist.html',
  styleUrl: './create-or-update-artist.scss',
})
export class CreateOrUpdateArtist implements OnInit {
  ...
  submit(): void {
    if (this.artistForm.valid) {
      ...
      const dod: string =
        this.artistForm.controls['yearOfDeath'].value === null
          ? ''
          : this.artistForm.controls['yearOfDeath'].value.toString();
      ...
      action$.subscribe(
        this._sharedObserver.getObserver(this.isUpdateMode(), messages, ['/all-artists']),
      );
    } else {
