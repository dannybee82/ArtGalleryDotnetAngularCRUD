...
import { Thumbnails } from '../../services/thumbnails/thumbnails';
...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, ReactiveFormsModule, AllMatModules, RouterLink],
  templateUrl: './create-painting.html',
  styleUrl: './create-painting.scss',
})
export class CreatePainting implements OnInit {
  ...
  private thumbnailsService = inject(Thumbnails);
  ...
  ngOnInit(): void {
    ...
    this.thumbnailForm.controls['selectedThumbnail'].valueChanges.subscribe(
      (thumbnail: Thumbnail) => {
        if (thumbnail) {
  ...
  deletePainting(): void {
    if (this._updatePainting()) {
      ...
      const dialog = this.dialog.open(DeleteDialog, { data: dialogData });
      ...
      complete: () => {
        if (this._updatePainting()) {
          ...
          if (this._updatePainting()?.thumbnail) {
            this.thumbnailForm.controls['selectedThumbnail'].setValue(
              this._updatePainting()!.thumbnail,
            );
